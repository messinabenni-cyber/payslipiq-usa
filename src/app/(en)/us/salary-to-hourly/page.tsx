...
const FAQS = [
  ...
  { q: 'Does this include taxes?', a: 'Yes. The right panel runs an annualised tax estimate using IRS Pub 15-T 2026 percentage method (Single), 6.2% Social Security up to the SSA 2026 wage base of $184,500, 1.45% Medicare, Additional Medicare 0.9% above $200,000, the most recently verified state rate, and employee-paid state programs where they apply. Optional local city or county tax is in the picker; default is none. Pre-tax 401(k)/HSA, dependents, and W-4 fine-tuning are not modelled. Use the Gross to Net Paycheck Calculator for per-paycheck numbers.' },
  ...
  return (
    <main className="piq-container py-10 max-w-3xl">
      ...
      <aside className="mt-6 border-l-4 border-amber-300 bg-amber-50 px-4 py-3 text-[13px] leading-relaxed text-amber-900 rounded" role="note">
        <strong className="font-semibold">Important:</strong> Estimates only. Single-filer assumption, no 401(k)/HSA, no dependents. Local tax is optional and defaults to none.
      </aside>
