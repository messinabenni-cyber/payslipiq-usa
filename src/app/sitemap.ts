...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  for (const s of STATES) {
    out.push({ url: `${SITE}/us/${s.slug}`, changeFrequency: 'weekly', priority: 0.6 });
    out.push({ url: `${SITE}/us/${s.slug}/paycheck-calculator`, changeFrequency: 'weekly', priority: 0.7 });
    out.push({ url: `${SITE}/us/${s.slug}/pay-stub-checker`, changeFrequency: 'weekly', priority: 0.6 });
    out.push({ url: `${SITE}/us/${s.slug}/state-tax`, changeFrequency: 'weekly', priority: 0.5 });
    out.push({ url: `${SITE}/us/${s.slug}/overtime-pay`, changeFrequency: 'weekly', priority: 0.5 });
    out.push({ url: `${SITE}/us/${s.slug}/pay-stub-laws`, changeFrequency: 'weekly', priority: 0.5 });
    // PR #11 addition: programmatic gross-to-net per state (50 states + DC)
    out.push({ url: `${SITE}/us/gross-to-net-paycheck/${s.slug}`, changeFrequency: 'weekly', priority: 0.7 });
    // Wave 1: take-home-pay state index + per-salary pages
    out.push({ url: `${SITE}/us/take-home-pay/${s.slug}`, changeFrequency: 'weekly', priority: 0.6 });
    for (const sal of TAKE_HOME_SALARIES) {
      out.push({ url: `${SITE}/us/take-home-pay/${s.slug}/${sal}`, changeFrequency: 'monthly', priority: 0.55 });
    }
  }
